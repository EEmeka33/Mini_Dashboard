...
/**
 * Start Bluetooth device pairing in background (non-blocking)
 * This function spawns a background process to scan and pair with a new device
 * Results will be logged to /tmp/bt_pair_quick.log
 * Status will be written to /tmp/bt_pair_result.txt
 * 
 * Returns:
...
static inline int bluetooth_pair_device_start(void)
{
    ...
    if (pid == 0) {
        ...
        /* Redirect output to avoid interfering with UI */
        int dev_null = open("/dev/null", O_WRONLY);
        dup2(dev_null, STDOUT_FILENO);
        close(dev_null);
        
        /* Execute pairing script */
        execl("/home/pi5/dashboard/bt_pair_quick.sh", 
              "bt_pair_quick.sh", 
              (char *)NULL);
